...
**Export tab** — pick the SD card folder, i.e. its mount point on this
computer (e.g. `/Volumes/SDCARD` on a Mac), check the decks you want, click
**Export**. It writes the checked decks' (+ subdecks') due cards into
`<sd card>/system-due/` as a timestamped `.ndjson` file — the same wire
format and the same card-collection logic (`_collect_due_cards`) a device's
normal "Download today's cards" pull uses. On the device: **Anki → Load
today's cards from SD** picks up the newest file there and deletes it once
installed.

**Import tab** — same SD card folder, click **Import**. As you review on the
device, it appends one line per graded review or flag toggle into
`<sd card>/system-answers/<batch-id>.ndjson` (`AnkiStore::appendAnswerEvent()`
